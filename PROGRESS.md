...
- **v0.6.4（同日第三版，`reports/052`）**：技能词表没加词边界（rust←trust、scala←scalable，83%/87% 的标签无文本依据）+ 来源戳记「问了谁」不记「谁答的」（3,098 条启发式列表顶 LLM 章）。修代码、加 `scripts/clean_skill_noise.py` 修存量、发 0.6.4（PyPI ✅ tag ✅ Release+mcpb ✅），398 tests。公开快照用清洗脚本修后重传。
