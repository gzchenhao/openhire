...
- **v0.6.6（同日第五版，`reports/053`）**：飞书调研（7 agents/257 calls）→ 理想第一方适配器（772 条含 JD，无日期，行上如实标）；蔚来适配器写好但被 EdgeOne 567 拦、停用待领导定；已知未收录雇主登记表（10 家飞书雇主回门户 + 原因 + 可授权 scope）；三个新 Moka 租户；四条诚实修复。PyPI ✅ tag ✅ Release+mcpb ✅，525 tests。周刷新工作流加了两条幂等清洗步骤。
...
**待领导确认**
- 蔚来：网站本身公开 1,784 条带日期岗位，但 EdgeOne 拦我们的抓取器（567）。选项：a) 用 567 页上的 request ID 找蔚来加白（属雇主认领路径）；b) 继续停用。我默认 b。
- 「特征探测」政策：实现 agent 为了判断 567 是按 TLS 指纹拦的，试过一次改 cipher list 的诊断请求（拿到 200，**没有**放进代码）。审查者认为这一步已越过 020 的「碰到即停」。要不要明令禁止此类探测，请领导定。
- 地平线用 Wintalent（wecruit.hotjob.cn），列表是 POST，探查猜的路径 404，要单独一次边界测试。

## 2026-09-13（傍晚）— 收回一个已发布结论；术语统一为「智驾」；mcpservers.org 已提交

- **CVPR 文章的核心结论已公开收回。** 读者 @海洋 指出单标签分类不合理，我按多标签重算：
  具身侧 27→33、智驾侧 25→**37**，「具身反超」翻盘。68 篇里 27 篇跨类重叠（占四成）。
  **没动 earlytip 的 FC3 生产管线**（强制单标签的 prompt 在 `fc3/conf_core.py:136`），
  改为抓已发布归档页做规则匹配，零线上风险。
  **重算自己的毛病也一并公开**：关键词匹配不是 LLM 全文，「仿真数据与评测」7→19 明显过火，
  所以对外说法是「这个反超不稳健」，不是「智驾其实更多」。
- **文章已改**（领导拍板的方案）：新标题《CVPR 2026 智驾/具身 68 篇全筛：9 个方向的分布，和一个我收回的结论》，
  顶部加更正块，**原文一字未删**。服务端已核验上线。
  为什么值得改：这篇只有 4 赞同、不是流量资产，**修它是因为标题与自己的评论区打架是个把柄**，
  而「主动收回自己的结论」恰恰是 OpenHire 唯一护城河（数可信）的现场演示。
  该复用到月报页方法论章节与 Show HN 正文，**不另写「我错了」的文章**（稀释赛道、像表演）。
- **术语纠正（领导指出）：「自动驾驶」的缩写一律用「智驾」，绝不用「自驾」**——
  「自驾」在通用中文里先指自驾游。两篇已发布文章的存量已改（各自服务端核验残留 0），
  drafts/reports/PROGRESS 全量替换完毕。已写入长期记忆。
- **mcpservers.org 已提交**（免费档 $0，12 小时内审核，通知发到 globalheadhunter007@gmail.com）。
  分类选「搜索」——**该站并没有报告里说的 job-search 分类**，那条检索结论不实。
- **Docker PR #5055 / Cline #2501 没有催**：均为 9/11 开，只有 2 天，状态 MERGEABLE、无 CI 失败、
  纯排队。**两天就催大型 registry 维护者是负收益**，建议 9/18 后再跟进。
- **脉脉第二条已由领导发布**（带可归因外链，指向知乎具身文章）。基线存于 `drafts/growth-log.md`。
- **一个可能推翻昨日判断的信号**：知乎该文发布 3.5h 仅 **16 阅读**，而 boss 直聘那条**回答** 10h 有 165。
  **知乎文章的自然流量可能远低于回答**，24h 后确认；若成立，往后应回到「答高流量问题」为主。

## 2026-09-13（下午）— 按数据换赛道：知乎文章转向具身/智驾，并量出「自荐券不值钱」
...
- **真正的发现是话题错配**：领导号的历史最好成绩是《现在选择具身智能是否合适？》**1,283 阅读**（无券），
  《自动驾驶人才都在哪里？》393；而招聘类最好只有 257。**读者在具身/智驾，不在招聘**。
...
- **内容是我们独有的数据**：30 家具身/智驾公司 2,350 个在架岗位，按**雇主自己 ATS 里的发布日**算在架天数
...
- **下一步**：24h 后看自然数据 → 决定 2 张券投这篇还是留着；knowledge：往后知乎内容默认走具身/智驾赛道。
