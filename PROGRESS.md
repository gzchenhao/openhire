...
- **同岗多城市刷屏**：1,347 个 (公司,标题) 组合存在多条在架岗位，MongoDB 单个岗位铺 22 个城市、Databricks 15 个。
  用户 `--limit 5` 可能整屏是同一个岗位。数据本身正确（job_id / 申请链接各不同），是呈现问题。
  修法要动搜索语义（按 (公司,标题) 折叠 + 返回城市数），**需领导先拍板**。
