...
print("\n最快摘牌 6 家:"); [print(f"  {t['name'][:22]:24} n={t['n']:4} 中位{t['median']:4}天 超半年{t['stale']:3}% 近30天被动过{t['touched']:3}%") for t in tbl[:6]]
print("\n最久未摘 6 家:"); [print(f"  {t['name'][:22]:24} n={t['n']:4} 中位{t['median']:4}天 超半年{t['stale']:3}% 近30天被动过{t['touched']:3}%") for t in sorted(tbl,key=lambda x:-x['median'])[:6]]
